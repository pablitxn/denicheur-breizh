...
All jobs inherit the `denicheur-breizh-ci` resource group, so this project's pipelines remain serial. Each job is capped at 1000m CPU plus a 100m helper and 1 GiB memory plus a 128 MiB helper. Even while Kubernetes is still terminating a previous pod, the new reservation fits below the shared runner namespace's observed CPU and 5 GiB memory quotas instead of failing before the script starts. `TURBO_CONCURRENCY=1` also serializes package tasks inside `check-workspace`, matching the single-CPU runner allocation rather than making API, web and extension tests contend in one pod.
