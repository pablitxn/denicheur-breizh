...
All jobs inherit the `denicheur-breizh-ci` resource group. This deliberately serializes this project's pipelines because the shared runner namespace allows 2500m CPU while a job plus its helper reserves 2100m; removing the lock can make otherwise valid concurrent jobs fail before their scripts start.
