...
import { DetailStatus, fieldLabel, FieldStateList, FieldTimestamp } from "./fields";
...
export function Captures({ selectedId, onSelect, onNew }: { selectedId: string | null; onSelect: (id: string) => void; onNew: () => void }) {
  ...
  return <>
    ...
    {runs.isPending ? <p role="status">{t("loading")}</p> : runs.data?.items.length === 0 ? <EmptyState className="lab-empty"><div className="empty-orbit"><Plus size={32} aria-hidden /></div><h2>{t("noRuns")}</h2><p>{t("noRunsHelp")}</p><Button onClick={onNew}>{t("capture")}</Button></EmptyState> : <>
      ...
      {current && <RunDetail key={current} id={current} onSelect={onSelect} />}
    </>}
  </>;
}

function RunDetail({ id, onSelect }: { id: string; onSelect: (id: string) => void }) {
  const t = useCopy(); const locale = usePreferences((state) => state.locale);
  ...
  return <section className="run-detail">
    <div className="panel run-summary"><div className="section-toolbar"><div><span className="eyebrow">{run.request.source} / {run.request.provider === "xai" ? "xAI" : "Firecrawl"}</span><h2>{run.request.name}</h2></div><div className="actions"><StatusChip status={run.status} />{isActive(run) ? <Button disabled={action.isPending} onClick={() => action.mutate("cancel")}><Square size={14} aria-hidden />{t("stop")}</Button> : !run.costUnknown && ["interrupted", "cancelled", "partial", "blocked"].includes(run.status) ? <Button disabled={action.isPending} onClick={() => action.mutate("resume")}><Play size={14} aria-hidden />{t("resume")}</Button> : null}<a className="btn" href={runExportUrl(id)} download><Download size={15} aria-hidden />{t("export")}</a></div></div>
      ...
      {run.request.repair && <div className="repair-lineage"><strong>{t("repairLinked")}</strong><p>{t("repairScope")}</p><p>{t("repairInheritedTimes")}</p><Button onClick={() => onSelect(run.request.repair!.parentRunId)}>{t("repairParent")}</Button></div>}
    ...
    {run.request.provider === "firecrawl" && !isActive(run) && <RepairPanel run={run} onCreated={onSelect} />}
...
function ObservationDetail({ observation: item, onClose }: { observation: CaptureObservation; onClose: () => void }) {
  const t = useCopy(); const locale = usePreferences((state) => state.locale);
  const fieldName = (field: DataField) => fieldLabel(field, t);
  return <SettingsDialog title={item.data.title ?? item.externalId} closeLabel={t("close")} onClose={onClose} sections={[
    { id: "detail", label: t("details"), content: <><a className="btn" href={item.url} target="_blank" rel="noreferrer">{t("openListing")}<ArrowUpRight size={15} aria-hidden /></a><p className="muted">{t("observed")} {date(item.observedAt, locale)}</p>{Object.entries(item.data).filter(([key]) => key !== "description" && key !== "imageUrls").map(([field, value]) => <SettingsRow key={field} label={fieldName(field as DataField)}><div className="detail-value">{Array.isArray(value) ? value.join(" · ") : value ?? "—"}<FieldTimestamp field={field as DataField} states={item.fieldStates ?? {}} fieldObservedAt={item.fieldObservedAt} observedAt={item.observedAt} /></div></SettingsRow>)}<h3>{t("description")}</h3><FieldTimestamp field="description" states={item.fieldStates ?? {}} fieldObservedAt={item.fieldObservedAt} observedAt={item.observedAt} /><p className="description-text">{item.data.description ?? t("unknown")}</p>{item.missingFields.length > 0 && <p className="workflow-notice">{t("missing")}: {item.missingFields.map(fieldName).join(", ")}</p>}{item.absentFields.length > 0 && <p className="muted">{t("absent")}: {item.absentFields.map(fieldName).join(", ")}</p>}</> },
    { id: "images", label: `${t("images")} (${item.data.imageUrls?.length ?? 0})`, content: <><FieldTimestamp field="imageUrls" states={item.fieldStates ?? {}} fieldObservedAt={item.fieldObservedAt} observedAt={item.observedAt} /><div className="gallery">{item.data.imageUrls?.map((url, index) => <a href={url} key={url} target="_blank" rel="noreferrer"><img src={url} alt={`${t("images")} ${index + 1}`} loading="lazy" referrerPolicy="no-referrer" /></a>)}</div></> },
    { id: "field-states", label: t("fieldStates"), content: <FieldStateList states={item.fieldStates ?? {}} fieldObservedAt={item.fieldObservedAt} observedAt={item.observedAt} /> },
