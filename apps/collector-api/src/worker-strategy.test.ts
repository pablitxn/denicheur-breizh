...
describe("immutable capture strategy and model", () => {
  it("registers the versioned Firecrawl strategies and can recreate the exact saved xAI model without making a request", () => {
    ...
    expect(registry.choices("firecrawl").map(choice => choice.id)).toEqual(["firecrawl-agent-scrape-v1", "firecrawl-agent-native-v2", "firecrawl-agent-expanded-v3", "firecrawl-detail-repair-v4", "firecrawl-native-inventory-v5", "firecrawl-gallery-audit-v6", "firecrawl-gallery-walk-v7"]);
  ...
  it.each(["firecrawl-agent-native-v2", "firecrawl-agent-expanded-v3", "firecrawl-detail-repair-v4", "firecrawl-native-inventory-v5", "firecrawl-gallery-audit-v6", "firecrawl-gallery-walk-v7"] as const)("dispatches an explicitly selected %s and persists that selection", async selectedStrategy => {
  ...
  it.each(["firecrawl-detail-repair-v4", "firecrawl-native-inventory-v5", "firecrawl-gallery-audit-v6", "firecrawl-gallery-walk-v7"])("%s visits the detail even when discovery claims a fully captured card", async strategy => {
    const selected = provider(strategy, "spark-2");
    const stages: string[] = [];
    selected.step = vi.fn(async context => {
      stages.push(context.work.kind);
      return { observations:[observationInputSchema.parse({url, detailStatus:"captured", data:{title:"Maison"},
        absentFields:dataFields.filter(field => field !== "title"), evidence:[{url,kind:"page",text:"Fixture listing: Maison; other fields absent."}]} )],
        nextPages:[], exhausted:context.work.kind === "discover", warnings:[], usage:{amount:1,unit:"credits" as const} };
    });
    const {store, worker} = fixture(selected);
    const run = worker.create(captureRequestSchema.parse({provider:"firecrawl", mode:"search", name:"Untrusted complete card", filters:{location:"Lannion"}}), `cards-${strategy}`);
    await worker.drain();
    expect(stages).toEqual(["discover","details"]);
    expect(store.getRun(run.id)).toMatchObject({discovered:1,captured:1,pending:0});
  });
