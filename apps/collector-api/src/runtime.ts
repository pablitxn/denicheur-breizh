...
export function createProviderStrategyRegistry(config:CollectorConfig):ProviderStrategyRegistry {
  ...
  return {
    choices(id){return id==="xai"?[{id:"xai-web-search-v1",label:"Hosted web search (v1)"}]:[{id:"firecrawl-agent-scrape-v1",label:"Agent + scrape (v1)"},{id:"firecrawl-agent-native-v2",label:"Native homepage flow (v2)"},{id:"firecrawl-agent-expanded-v3",label:"Agent + expanded detail (v3)"},{id:"firecrawl-detail-repair-v4",label:"Dedicated details and field repair (v4)"},{id:"firecrawl-native-inventory-v5",label:"Native price and full gallery inventory (v5)"},{id:"firecrawl-gallery-audit-v6",label:"Native inventory and gallery dialog audit (v6)"},{id:"firecrawl-gallery-walk-v7",label:"Complete native gallery traversal (v7)"}];},
    resolve(id,strategy,model){
      const supported=id==="xai"?strategy==="xai-web-search-v1":["firecrawl-agent-scrape-v1","firecrawl-agent-native-v2","firecrawl-agent-expanded-v3","firecrawl-detail-repair-v4","firecrawl-native-inventory-v5","firecrawl-gallery-audit-v6","firecrawl-gallery-walk-v7"].includes(strategy);
...
export function createRuntime(config:CollectorConfig=readConfig(),providers?:Map<ProviderId,CaptureProvider>){
  ...
  const registry=providers??new Map<ProviderId,CaptureProvider>([["xai",strategies!.resolve("xai","xai-web-search-v1")],["firecrawl",strategies!.resolve("firecrawl","firecrawl-gallery-walk-v7")]]);
