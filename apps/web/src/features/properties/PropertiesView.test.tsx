...
describe("PropertiesView", () => {
  it.each([
    { column: "Prix", field: "priceEuros" },
    { column: "Évaluation", field: "score" },
  ] as const)("keeps missing values last when sorting $column in either direction", async ({ column, field }) => {
    const listings = [
      listing("Unknown", field === "score" ? { evaluation: evaluation(null) } : {}),
      listing("Low", field === "score" ? { evaluation: evaluation(20) } : { priceEuros: 100_000 }),
      listing("High", field === "score" ? { evaluation: evaluation(90) } : { priceEuros: 300_000 }),
      listing("Zero", field === "score" ? { evaluation: evaluation(0) } : { priceEuros: 0 }),
    ];
    installApi(listings);
    renderProperties();
    ...
    expect(rowTitles(table)).toEqual(["High", "Low", "Zero", "Unknown"]);

    fireEvent.click(within(table).getByRole("button", { name: column }));
    expect(rowTitles(table)).toEqual(["Zero", "Low", "High", "Unknown"]);
  });
  ...
  it.each(["table", "cards"] as const)("keeps cached %s results usable after a refresh failure and recovers on retry", async (mode) => {
    ...
    const selection = screen.getByRole("button", {
      name: mode === "table" ? /Maison du port.*leboncoin-Maison du port/u : "Afficher le détail de Maison du port",
    });
...
function installApi(listings: PropertyListing[]) {
  return {
    list: vi.spyOn(denicheurApi, "listAllProperties").mockResolvedValue({ items: listings, total: listings.length, nextCursor: null }),
...
function evaluation(score: number | null): NonNullable<PropertyListing["evaluation"]> {
  return {
    listingId: "listing-1",
    runId: "run-1",
    decision: "review",
